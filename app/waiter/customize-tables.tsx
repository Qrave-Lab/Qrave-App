...
export default function CustomizeTables() {
  ...
  const handleFreeTable = async (tableId: string) => {
    ...
    Alert.alert(
      "Free Table",
      `Free Table ${tableNumber ?? tableId}? This will cancel all active orders for this table.`,
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              // Cancel all active orders for this table on the backend
              const activeOrdersForTable = activeOrders.filter((o) => o.table_number === tableNumber && o.orderId);
              for (const order of activeOrdersForTable) {
                if (order.orderId) {
                  await api.patch(`/api/admin/orders/${order.orderId}/status`, { status: "cancelled" });
                }
              }

              // Also cancel any open sessions if such API exists (best-effort)
              try {
                await api.post(`/api/admin/tables/${tableId}/clear`, {});
              } catch (e) {
              ...
              Alert.alert("Error", e?.message || "Failed to free table on the server.");
            } finally {
              setFreeLoading(null);
            }
          },
  ...
  const isKitchenVisible = (a: Activity) =>
    a.channel === "kitchen" && a.status !== "served" && a.status !== "cancelled";
    ...
    <View style={styles.mainContainer}>
        ...
        <View style={styles.headerTopRow}>
          <View style={styles.headerCenter}>
            ...
            <View style={styles.headerLocationRow}>
              <MaterialIcons name="location-on" size={14} color="rgba(255,255,255,0.75)" />
      ...
      >
        {loading && (
          <View style={{ padding: 16, alignItems: "center" }}>
            ...
            <Text style={{ marginTop: 6, color: "#9CA3AF" }}>Loading tables...</Text>
          </View>
        )}
        {error && (
        ...
        <View style={styles.metricsRow}>
          <TouchableOpacity style={styles.metricCard} onPress={() => setFilter("all")}>
          ...
          <TouchableOpacity style={styles.metricCard}>
            <View style={[styles.metricIcon, { backgroundColor: "#DCFCE7" }]}>
              <MaterialIcons name="receipt-long" size={18} color={WaiterColors.primaryDark} />
            </View>
        ...
        >
          ...
          >
            <MaterialIcons name="grid-view" size={14} color={filter === "all" ? "#FFF" : "#6B7280"} style={{ marginRight: 4 }} />
            <Text style={[styles.filterChipText, filter === "all" && styles.filterChipTextActive]}>
              All Tables
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.filterChip, filter === "occupied" && styles.filterChipActive]}
            onPress={() => setFilter("occupied")}
          >
            <MaterialIcons name="people" size={14} color={filter === "occupied" ? "#FFF" : "#6B7280"} style={{ marginRight: 4 }} />
            <Text style={[styles.filterChipText, filter === "occupied" && styles.filterChipTextActive]}>
          ...
          >
            <MaterialIcons name="event-available" size={14} color={filter === "free" ? "#FFF" : "#6B7280"} style={{ marginRight: 4 }} />
              ...
              >
                ...
                {isFree ? (
                  ...
                  <View style={styles.emptyStateContainer}>
                    <MaterialIcons name="table-restaurant" size={32} color="#D1D5DB" />
      ...
      >
        <View style={styles.bottomSheetOverlay}>
          <TouchableOpacity style={{ flex: 1 }} activeOpacity={1} onPress={() => setActionSheetOpen(false)} />
          <View style={styles.bottomSheet}>
            <View style={styles.sheetHeader}>
              <View>
                <Text style={styles.sheetTitle}>
                  Table {actionTableNumber !== undefined ? padNumber(actionTableNumber) : "?"} - Manage
                </Text>
                {actionTableBg && actionTableBg.linkedTableNumbers.length > 1 && (
                  <Text style={styles.sheetSubtitle}>
                    🔗 Merged with T{actionTableBg.linkedTableNumbers.filter((n) => n !== actionTableNumber).join(", T")}
                  </Text>
                )}
              </View>
              <TouchableOpacity style={styles.closeBtn} onPress={() => setActionSheetOpen(false)}>
                <MaterialIcons name="close" size={18} color="#6B7280" />
              </TouchableOpacity>
            </View>

            {/* Summary bar */}
            {actionTable && actionTable.status === "occupied" && (
              <View style={styles.statusCard}>
                ...
                <View style={styles.statusDetails}>
                  <View style={styles.statusDetailRow}>
                    <MaterialIcons name="restaurant" size={14} color="#6B7280" />
                    <Text style={styles.statusDetailText}>{actionTable?.items || 0} Items</Text>
                  </View>
                  <View style={styles.statusDetailRow}>
                    <MaterialIcons name="schedule" size={14} color="#6B7280" />
            ...
            <View style={styles.actionGrid}>
              {/* Place Order — always available */}
              <TouchableOpacity
                style={[styles.actionBtnStyle, styles.actionBtnOrder]}
                onPress={() => {
                  setActionSheetOpen(false);
                  if (actionTable) handleOpenTakeOrder(actionTable);
                }}
              >
                <MaterialIcons name="add-shopping-cart" size={24} color={WaiterColors.primaryDark} />
                <Text style={styles.actionBtnOrderText}>Place Order</Text>
              </TouchableOpacity>
              ...
              {actionTable && actionTable.status === "occupied" && (
                <>
                  ...
                  >
                    <MaterialIcons name="payments" size={24} color={WaiterColors.primaryDark} />
      ...
      >
        <View style={styles.bottomSheetOverlay}>
          <TouchableOpacity style={{ flex: 1 }} activeOpacity={1} onPress={() => setActivitySheetOpen(false)} />
          <View style={styles.bottomSheet}>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>Activity Feed</Text>
              <TouchableOpacity style={styles.closeBtn} onPress={() => setActivitySheetOpen(false)}>
                <MaterialIcons name="close" size={18} color="#6B7280" />
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.modalTabsRow}>
              <TouchableOpacity
                style={[styles.modalTabBtn, activityTab === "kitchen" && styles.modalTabBtnActive]}
                onPress={() => setActivityTab("kitchen")}
              >
                <Text style={[styles.modalTabLabel, activityTab === "kitchen" && styles.modalTabLabelActive]}>
                  Kitchen({kitchenCount})
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.modalTabBtn, activityTab === "service" && styles.modalTabBtnActive]}
                onPress={() => setActivityTab("service")}
              >
                <Text style={[styles.modalTabLabel, activityTab === "service" && styles.modalTabLabelActive]}>
            ...
            {activityLoading ? (
              ...
              <ScrollView style={{ maxHeight: 400 }} showsVerticalScrollIndicator={false}>
                {activities.filter((a) =>
                  activityTab === "kitchen" ? isKitchenVisible(a) : isServiceVisible(a),
                ).length === 0 ? (
                  <View style={styles.emptyState}>
                      ...
                      <View key={a.id} style={styles.activityCard}>
                        <View style={styles.activityCardHeader}>
                          <View style={styles.activityBadge}>
                            <Text style={styles.activityBadgeText}>{a.table}</Text>
                          </View>
                              ...
                              {a.status === "pending" ? (
                                <>
                                  <TouchableOpacity
                                    ...
                                    onPress={() => handleKitchenStatus(a.orderId, "cancelled")}
                                  >
                                    <Text style={styles.actBtnRejectText}>Reject</Text>
                                  </TouchableOpacity>
                                  <TouchableOpacity
                                    style={[styles.actBtn, styles.actBtnAccept]}
                                    onPress={() => handleKitchenStatus(a.orderId, "accepted")}
                                  >
                                    <Text style={styles.actBtnAcceptText}>Accept</Text>
                                  </TouchableOpacity>
                                </>
                              ) : (
                                <TouchableOpacity
                                  style={[styles.actBtn, styles.actBtnServe]}
                                  onPress={() => handleKitchenStatus(a.orderId, "served")}
                                >
                                  <Text style={styles.actBtnServeText}>Mark Served</Text>
                                </TouchableOpacity>
                              )}
                            </>
                          ) : (
                            <>
                              {a.status === "open" ? (
                                <>
                                  <TouchableOpacity
                                    style={[styles.actBtn, styles.actBtnReject]}
                                    onPress={() => handleServiceStatus(a.serviceId, "done")}
                                  >
                                    <Text style={styles.actBtnRejectText}>Done</Text>
                                  </TouchableOpacity>
                                  <TouchableOpacity
                                    style={[styles.actBtn, styles.actBtnAccept]}
                                    onPress={() => handleServiceStatus(a.serviceId, "attending")}
                                  >
                                    <Text style={styles.actBtnAcceptText}>Attend</Text>
                                  </TouchableOpacity>
                                </>
                              ) : (
                                <TouchableOpacity
                                  ...
                                  onPress={() => handleServiceStatus(a.serviceId, "done")}
                                >
                                  <Text style={styles.actBtnServeText}>Resolve</Text>
                                </TouchableOpacity>
                              )}
                            </>
                          )}
          ...
          <View style={styles.dialogBox}>
            ...
            <Text style={{ marginBottom: 8, color: "#666" }}>
              {(() => {
                ...
                const label = src ? `T${src.number ?? getTableNumber(src) ?? src.id}` : mergeSource;
                return `Merging ${label} into another occupied table`;
              })()}
            </Text>
            {mergeError ? <Text style={{ color: "red", marginBottom: 10 }}>{mergeError}</Text> : null}
            {mergeLoading ? (
              <View style={{ padding: 24, alignItems: "center" }}>
                <ActivityIndicator size="large" color={WaiterColors.primary} />
                <Text style={{ marginTop: 10, color: "#6B7280" }}>Merging bills...</Text>
              </View>
            ) : (
              <FlatList
                data={tablesWithOrders.filter((t) => t.id !== mergeSource && t.status === "occupied")}
                keyExtractor={(t) => t.id}
                renderItem={({ item }) => (
                  <TouchableOpacity style={styles.dialogItem} onPress={() => handleMergeTable(item.id)}>
                    <Text style={styles.dialogItemText}>
                      T{item.number ?? getTableNumber(item) ?? item.id} - {item.total}
                    </Text>
                ...
                ListEmptyComponent={
                  <View style={{ paddingVertical: 16, alignItems: "center" }}>
                    <Text style={{ color: "#9CA3AF" }}>No other occupied tables to merge with.</Text>
                  </View>
                }
              />
            )}
            <TouchableOpacity
              onPress={() => { if (!mergeLoading) { setMergeModalOpen(false); setMergeError(null); } }}
          ...
          <View style={styles.dialogBox}>
            ...
            <Text style={{ marginBottom: 8, color: "#666" }}>
              {(() => {
                ...
                const label = src ? `T${src.number ?? getTableNumber(src) ?? src.id}` : moveSource;
                return `Moving ${label} to another table`;
              })()}
            </Text>
            {moveError ? <Text style={{ color: "red", marginBottom: 10 }}>{moveError}</Text> : null}
            <FlatList
              data={tablesWithOrders.filter((t) => t.id !== moveSource && t.status === "free")}
              keyExtractor={(t) => t.id}
              renderItem={({ item }) => (
                <TouchableOpacity
                  style={styles.dialogItem}
                  onPress={() => {
                    if (!item.tableId) { setMoveError("Target table id missing. Please refresh table data."); return; }
                    handleMoveTable(item.tableId);
                  }}
                  disabled={moveLoading}
                >
                  <Text style={styles.dialogItemText}>T{item.number ?? getTableNumber(item) ?? item.id} (Free)</Text>
                  <MaterialIcons name="arrow-forward" size={20} color="#666" />
                </TouchableOpacity>
              )}
              ListEmptyComponent={
                <View style={{ paddingVertical: 16, alignItems: "center" }}>
                  <Text style={{ color: "#9CA3AF" }}>No empty tables available.</Text>
                </View>
              }
            />
            <TouchableOpacity onPress={() => setMoveModalOpen(false)} style={styles.dialogCloseBtn}>
              <Text style={{ color: "#FFF", fontWeight: "700" }}>Close</Text>
            </TouchableOpacity>
          </View>
        </View>
      ...
      >
        <View style={styles.centeredOverlay}>
          <View style={styles.dialogBox}>
            ...
            <Text style={{ marginBottom: 20, color: "#666" }}>Export bill for table?</Text>
            <TouchableOpacity
              style={[styles.dialogCloseBtn, { backgroundColor: WaiterColors.primary }]}
              onPress={async () => {
                const t = tablesData.find((x) => x.id === printSource);
                if (t) {
                  const csv = `Table,Items,Total\n${t.id},${t.items},${t.total}\n`;
                  try { await Share.share({ title: `Bill_${t.id}.csv`, message: csv }); } catch {}
                }
            ...
            <TouchableOpacity
              ...
              style={[styles.dialogCloseBtn, { marginTop: 10, backgroundColor: "#9CA3AF" }]}
            >
      ...
      <Modal
        ...
        animationType="slide"
        statusBarTranslucent
        onRequestClose={() => { if (!paidLoading) { setPaidModalOpen(false); setPaidSource(null); } }}
      >
        <View style={styles.bottomSheetOverlay}>
          <TouchableOpacity
            style={{ flex: 1 }}
            activeOpacity={1}
            onPress={() => { if (!paidLoading) { setPaidModalOpen(false); setPaidSource(null); } }}
          />
          <View style={styles.bottomSheet}>
            <View style={styles.sheetHeader}>
                ...
                <>
                  ...
                  {bg && (
                    <View style={[styles.mergedBadge, { marginBottom: 8 }]}>
                      <Text style={styles.mergedBadgeText}>
                        🔗 Combined bill with T{bg.linkedTableNumbers.filter((n) => n !== tblNumber).join(", T")}
                      </Text>
                    </View>
                  )}
                  ...
                  {bg ? (
                    <Text style={{ marginBottom: 16, color: WaiterColors.primaryDark, fontWeight: "600", fontSize: 12 }}>
                      Paying will settle & free all {bg.linkedTableNumbers.length} tables
                    </Text>
                  ) : (
            ...
            {paidLoading ? (
              ...
              <View>
                <Text style={{ marginBottom: 12, fontWeight: "700", color: "#111" }}>Select Payment Mode</Text>
                {(["cash", "card", "upi"] as const).map((mode) => (
                  ...
                  >
                    <View style={styles.paymentModeIcon}>
                      <MaterialIcons
                        name={mode === "cash" ? "payments" : mode === "card" ? "credit-card" : "phone-android"}
                        size={22}
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.paymentModeSub}>
                        {mode === "cash" ? "Pay with cash" : mode === "card" ? "Debit/Credit card" : "Google Pay, PhonePe, etc."}
                      </Text>
                    </View>
...
const styles = StyleSheet.create({
  ...
  sortOptionActive: { backgroundColor: "#DCFCE7" },
  sortOptionText: { fontSize: 12, fontWeight: "600", color: "#6B7280" },
  sortOptionTextActive: { color: WaiterColors.primaryDark, fontWeight: "800" },
  ...
  sheetSubtitle: { fontSize: 13, color: "#6B7280", fontWeight: "500", marginTop: 2 },
  closeBtn: {
    width: 32,
    height: 32,
    borderRadius: 16,
  ...
  statusMain: { flex: 1 },
  statusLabel: { fontSize: 12, color: "#6B7280", fontWeight: "600", marginBottom: 4 },
  statusValue: { fontSize: 24, fontWeight: "800", color: "#111", letterSpacing: -0.5 },
  verticalDivider: { width: 1, height: "80%", backgroundColor: WaiterColors.border, marginHorizontal: 16 },
  statusDetails: { justifyContent: "center", gap: 8 },
  statusDetailRow: { flexDirection: "row", alignItems: "center", gap: 6 },
  statusDetailText: { fontSize: 13, fontWeight: "600", color: "#4B5563" },
  sectionLabel: {
    fontSize: 13,
    fontWeight: "700",
    color: "#9CA3AF",
  ...
  dangerBtnText: { fontSize: 14, fontWeight: "700", color: "#EF4444", marginLeft: 8 },
  ...
  modalTabBtn: { flex: 1, paddingVertical: 8, alignItems: "center", borderRadius: 10 },
  modalTabBtnActive: {
    backgroundColor: "#FFF",
    shadowColor: "#000",
    shadowOpacity: 0.05,
  ...
  modalTabLabel: { fontWeight: "600", color: "#6B7280", fontSize: 14 },
  modalTabLabelActive: { color: WaiterColors.primaryDark, fontWeight: "700" },
  ...
  activityCardHeader: { flexDirection: "row", alignItems: "center", marginBottom: 10 },
  activityBadge: {
    width: 40,
    height: 40,
    borderRadius: 10,
  ...
  activityBadgeText: { fontWeight: "800", color: WaiterColors.primaryDark, fontSize: 14 },
  activityTitle: { fontSize: 15, fontWeight: "700", color: "#333" },
  activityNote: { fontSize: 12, color: "#999", marginTop: 1 },
  statusChip: {
    backgroundColor: "#DCFCE7",
    paddingHorizontal: 8,
    paddingVertical: 3,
  ...
  activityActions: { flexDirection: "row", justifyContent: "flex-end", gap: 8 },
  actBtn: { paddingVertical: 8, paddingHorizontal: 16, borderRadius: 8, minWidth: 80, alignItems: "center" },
  actBtnReject: { backgroundColor: "#FEF2F2" },
  actBtnRejectText: { color: "#DC2626", fontWeight: "700", fontSize: 13 },
  actBtnAccept: { backgroundColor: "#DCFCE7" },
  actBtnAcceptText: { color: WaiterColors.primaryDark, fontWeight: "700", fontSize: 13 },
  actBtnServe: { backgroundColor: "#EFF6FF" },
  actBtnServeText: { color: "#2563EB", fontWeight: "700", fontSize: 13 },
  ...
  dialogTitle: { fontSize: 20, fontWeight: "800", color: "#111", marginBottom: 8 },
  dialogItem: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
  ...
  dialogItemText: { fontSize: 16, color: "#333", fontWeight: "500" },
  dialogCloseBtn: {
    marginTop: 20,
  ...
  paymentModeLabel: { fontWeight: "700", fontSize: 15, color: "#111827" },
  paymentModeSub: { fontSize: 12, color: "#6B7280", marginTop: 2 },
});
